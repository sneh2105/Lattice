...
def test_html_report_generated():
    ...
    path = generate_html_report(result, f"{out_dir}/report.html")
...
def test_html_report_escapes_html_special_chars():
    ...
    result = scan_agent_config(config)
    out_dir = tempfile.mkdtemp()
    path = generate_html_report(result, f"{out_dir}/report.html", title="<script>alert(1)</script>")
    content = Path(path).read_text()
...
def test_html_report_path_adds_extension():
    ...
    result = scan_agent_config(config)
    out_dir = tempfile.mkdtemp()
    path = generate_html_report(result, f"{out_dir}/report")  # no extension
...
def test_cli_html_output_flag():
    ...
    config = make_dangerous_config()
    out_dir = tempfile.mkdtemp()
    out_file = f"{out_dir}/cli_report.html"
